...
info= {
    ...
    'wave_len'          : 2000,  #dac输出点数，采样率2G，2000代表1000ns
    'cycle_times'       : 1,   #实验循环次数; 频谱仪测试时用1；
    'cycle_period_ns'   : 1000,   # 设置为 wave_len相同时是连续波； 实验循环周期一般用 <=200us
...
for name, da in driver.da.items():
    if True:
        print(name)
        driver.da_set_single_waveform(name=name, iq_channel_select='i',wave=wave_sin,play_mode='cycle_play')
        driver.da_set_single_waveform(name=name, iq_channel_select='q',wave=wave_cos,play_mode='cycle_play')
