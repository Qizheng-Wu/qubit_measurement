...
info= {
    ...
    'dac_amp'           : 0.025,   #dac的输出幅值，AD/DA直连情况下建议不要超过0.2
    # 'phase_list'        : [0], #相位扫描列表
...
dac_name = 'da_box1pcie6ch12'
adc_name = 'ad_box1pcie11ch12'
...
for i in range(len(info['freq_list'])):
    demo_cos,demo_sin = driver.tools.gen_normalized_demodulation_factor(IF_freq=info['freq_list'][i],demo_length=info['adc_window_len'],demod_width=100)
    # plt.plot(demo_cos)

    #设定adc解模因子
    if with_mixer:
        driver.ad_set_demodulation_factor(name=adc_name,freq_ch=i,demo_i=demo_sin,demo_q=demo_cos)
        info[f'i_demo{i+1}'] = demo_sin.tolist()
        info[f'q_demo{i+1}'] = demo_cos.tolist()
    else:
        driver.ad_set_demodulation_factor(name=adc_name,freq_ch=i,demo_i=demo_cos,demo_q=demo_sin)
        info[f'i_demo{i+1}'] = demo_cos.tolist()
        info[f'q_demo{i+1}'] = demo_sin.tolist()
...
for phase in info['phase_list']:
    ...
    wave_cos = driver.tools.gen_multi_tone_wave(wave_shape='cos',freq_list=info['freq_list'],play_mode='end_with_zero',
                                                phase_offset_list=[phase,0],amp_list=info['amp_list'],wave_len=2000,max_amplitude=info['dac_amp'],pulse_width = 100)
    wave_sin = driver.tools.gen_multi_tone_wave(wave_shape='sin',freq_list=info['freq_list'],play_mode='end_with_zero',
                                            phase_offset_list=[phase,0],amp_list=info['amp_list'],wave_len=2000,max_amplitude=info['dac_amp'],pulse_width = 100)
    ...
    info['i_raw_data'].extend(raw_data_i.tolist())
    info['q_raw_data'].extend(raw_data_q.tolist())
    ...
    info['i_freq2_sum_fpga'].extend(i_sum[1].tolist())#将每次的I_sum逐一记录
    info['q_freq2_sum_fpga'].extend(q_sum[1].tolist())
