...
total_iq_times = 680_000
cycle_times = 1000
phase_len= int(total_iq_times/cycle_times)
...
info= {
    ...
    'cycle_times'       : cycle_times,   #实验循环次数
    'cycle_period_ns'   : 5000,   #实验循环周期
    'dac_amp'           : 0.2,   #dac的输出幅值，AD/DA直连情况下建议不要超过0.2
    'phase_list'        : list(np.linspace(0,360,phase_len)), #相位扫描列表
    #储存所有的IQ结果
...
print("shape of i_sum:",i_sum.shape)
#print('i_sum=',i_sum)
...
plt.plot(info2['i_freq1_sum_fpga'])
plt.plot(info2['q_freq1_sum_fpga'])
plt.show()
plt.pause()