...
wave = driver.tools.gen_single_tone_wave(wave_shape='square',frequency=info['IF_frq'],\
                                        ...
                                        wave_len=info['wave_len'],amplitude=info['dac_amp'])/2+info['dac_amp']/2
...
for i in range(10000):
    ...
    for name, da in driver.da.items():
        ...
        driver.da_set_single_waveform(name=name, iq_channel_select='i',wave=wave,play_mode='end_with_zero')
        driver.da_set_single_waveform(name=name, iq_channel_select='q',wave=wave,play_mode='end_with_zero')
