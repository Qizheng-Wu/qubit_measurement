'''
(未经验证，请勿参考)检查背板的锁定功能,尝试在背板运行工程中改变循环次数看是否能够成功.
'''
...
for name, da in driver.da.items():
    ...
    driver.da_set_level2_trigger_ram(name=name,time_stamp_list_ns=[20],cmd_list=[1])
...
print("尝试运行中改变循环次数,看是否能够成功")
driver.sys_clear_all_level2_trigger_ram()#清空所有二级trigger ram
driver.sys_stop_all_borad() #给所有二级trigger ram写入一次停止命令, 并向所有板卡发送"停止"trigger
print("尝试结束")

# print("尝试强行终止循环")
# driver.sys_unlock_all_backplane() #强行解锁所有背板
# driver.sys_clear_all_level2_trigger_ram()#清空所有二级trigger ram
# driver.sys_stop_all_borad() #给所有二级trigger ram写入一次停止命令, 并向所有板卡发送"停止"trigger
# print("尝试结束")
