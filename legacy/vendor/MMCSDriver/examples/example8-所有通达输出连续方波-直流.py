...
info= {
    ...
    'cycle_times'       : 100000,   #实验循环次数
    'cycle_period_ns'   :100_000,   #实验循环周期
    'dac_amp'           :0.9,   #dac的输出幅值，AD/DA直连情况下建议不要超过0.2
...
for name, da in driver.da.items():
    # if name in ['da_box2pcie14ch34']:
    # if name[:7] in ['da_box2','da_box3']:
    print(name)

    driver.da_set_single_waveform(name=name, iq_channel_select='i',wave=wave,play_mode='end_with_keep')
    driver.da_set_single_waveform(name=name, iq_channel_select='q',wave=wave,play_mode='end_with_keep')
