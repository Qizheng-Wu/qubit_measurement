...
info= {
    ...
    'i_freq1_sum_fpga'        : [],
    'q_freq1_sum_fpga'        : [],
    'i_freq1_sum_fpga_std'        : [],
    'q_freq1_sum_fpga_std'        : [],
...
with_mixer = True #是否使用板卡自带的mixer(针对带有mixer的DACxy板卡和ADC板卡请设置为True) #需要额外设定的原因是板卡自带的mixer与外接mixer的IQ定义相反
scan_type = 'dac' #dac 扫描adc/dac的tap
...
phase = 0
#产生解模因子
demo_cos,demo_sin = driver.tools.gen_normalized_demodulation_factor(IF_freq=info['IF_frq'],demo_length=info['adc_window_len'],demo_phase=phase)
...
tap_list = np.array(range(32))
for tap in tap_list:
    if scan_type == 'adc':
        print('adc tap=',tap)
        driver.ad_set_trigger_delay(name=adc_name,delay_tap=int(tap))
    else:
        # driver.ad_set_trigger_delay(name=adc_name,delay_tap=int(15))
        print('dac tap=',tap)
        driver.da_set_trigger_delay(name=dac_name,delay_tap=int(tap))
    ...
    info['i_freq1_sum_fpga_std'].append(np.std(i_sum[0]))#将每次的I_sum逐一记录
    info['q_freq1_sum_fpga_std'].append(np.std(q_sum[0]))
...
driver.ad_set_trigger_delay(name=adc_name,delay_tap=int(0))
print('tap has been reset to 0')
...
plt.plot(tap_list,info['i_freq1_sum_fpga_std'],'o-')
plt.plot(tap_list,info['q_freq1_sum_fpga_std'],'o-')
if scan_type == 'adc':
    plt.xlabel('ADC tap')
else:
    plt.xlabel('DAC_tap')
plt.ylabel('IQ std')
plt.title(f"IQ stability {adc_name} {dac_name}")
plt.show()