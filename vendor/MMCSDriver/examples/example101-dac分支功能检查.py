...
info= {
    ...
    'wave_len'          :200,  #dac输出点数
    'cycle_times'       : 1,   #实验循环次数
    'cycle_period_ns'   :5000,   #实验循环周期
    'dac_amp'           :0.8,   #dac的输出幅值，AD/DA直连情况下建议不要超过0.2
...
#生成波形：
wave0 = driver.tools.gen_single_tone_wave(wave_shape='sin',frequency=info['IF_frq'],\
                                                play_mode='end_with_zero',phase_offset=0,
                                                wave_len=info['wave_len'],amplitude=info['dac_amp'])

wave1 = driver.tools.gen_single_tone_wave(wave_shape='square',frequency=info['IF_frq'],\
                                                play_mode='end_with_zero',phase_offset=0,
                                                wave_len=info['wave_len'],amplitude=info['dac_amp'])
...
wave3 = driver.tools.gen_single_tone_wave(wave_shape='cos',frequency=info['IF_frq'],\
                                                play_mode='end_with_zero',phase_offset=0,
                                                wave_len=info['wave_len'],amplitude=info['dac_amp'])

waveform = [wave0,wave1,wave2,wave3]

#准备波形列表
playlist = [{'trigger':driver.trigger_start,    'wave_idx'      :0,     }, #当dac接收到trigger_start时，播放waveform[0]
            {'trigger':driver.trigger_feedback, 'branch1_idx'   :2,     'branch0_idx'   :1,     }, #当dac接收到trigger_feedback时，播放waveform[2]或者waveform[1]
            {'trigger':driver.trigger_start,    'wave_idx'      :3,     }] #当dac接收到trigger_start时，播放waveform[3]

#设定DAC波形
for name, da in driver.da.items():

    driver.da_set_multi_waveform(name=name, iq_channel_select='i',play_mode='end_with_zero',
                                 waveform=waveform,playlist=playlist)
    driver.da_set_multi_waveform(name=name, iq_channel_select='q',play_mode='end_with_zero',
                                 waveform=waveform,playlist=playlist)
    
    #设定背板中对应这个DAC通道的二级trigger ram,只有开始没有停止命令, 第2个可以是4或者8
    driver.da_set_level2_trigger_ram(name=name,
                                     time_stamp_list_ns=[20,200,400],
                                     cmd_list=[driver.trigger_start,driver.trigger_branch0,driver.trigger_start]) #强制播放branch0
                                    #  cmd_list=[driver.trigger_start,driver.trigger_branch1,driver.trigger_start]) #强制播放branch1
