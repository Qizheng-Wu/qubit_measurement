...
for phase in range(0,360,360):#开始实验设定
    ...
    driver.da_set_trigger(name='da_box1pcie8ch12',time_stamp_list=[16e-9,1100e-9,2200e-9,4000e-9],cmd_list=[1,2,1,2])#设定DAC trigger
   
    driver.ad_clear_data(name='ad_box1pcie6ch34')#清空ADC
    
    driver.ad_set_sample(name='ad_box1pcie6ch34',sample_len=1200,cycle_times = 1)#设定ADC 解模/采样参数
    
    driver.ad_set_trigger(name='ad_box1pcie6ch34',time_stamp_list=[16e-9,2200e-9],cmd_list=[1,1])#设定ADC trigger

    driver.sys_set_parameter(cycle_times=1,cycle_period=4e-6)#设定总系统参数
