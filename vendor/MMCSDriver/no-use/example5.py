...
for phase in range(0,360,360):#开始实验设定
    ...
    driver.ad_clear_data(name='ad_box1pcie6ch12')#清空ADC
    
    driver.ad_set_sample(name='ad_box1pcie6ch12',sample_len=1500,cycle_times = 1)#设定ADC 采样参数
    
    driver.ad_set_trigger(name='ad_box1pcie6ch12',time_stamp_list=[16e-9],cmd_list=[1])#设定ADC trigger

    driver.sys_set_parameter(cycle_times=1,cycle_period=2.5e-6)#设定总系统参数
    
    driver.sys_run_trigger(master_box_name='box1')#开始运行直到结束
    
    raw_data_i, raw_data_q= driver.ad_get_rawdata(name='ad_box1pcie6ch12')#回传ad裸数据
